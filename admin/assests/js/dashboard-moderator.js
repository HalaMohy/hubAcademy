document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function getCategore() {
        try {
            const response = await fetch("https://localhost:7170/api/Account/PendingModerator", {
                method: 'GET',
                headers: {
            ...
            Categorys.forEach((c) => {
                ...
                const certificatesButtons = c.certificates.map((e) => {
                    return `
            <a class="btn" href="./assets/image/${e.fileName}" target="_blank">
...
<i class="fa-solid fa-user-check"></i>
    ...
    async function approvedModerator(selectId){
        try{
            ...
            const response =await fetch(`https://localhost:7170/api/Account/ApprovegModerator/${selectId}`,{
                 method: 'PUT',
                headers: {
                    "Authorization": `Bearer ${token}`,
                },
